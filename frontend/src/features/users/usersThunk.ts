...
export const googleLoginOrRegisterUser = createAsyncThunk<
  IUser,
  string,
...
>(
  ...
  async (credential, { rejectWithValue }) => {
    try {
      const response = await axiosRequest.post<UserLoginResponse>(
        "/users/google",
        { credential },
      );
...
>("users/registerUser", async (user, { rejectWithValue }) => {
  try {
    ...
    const response = await axiosRequest.post<IUser>(
      "/users/register",
      formData,
    );
...
export const logoutUser = createAsyncThunk<void, void, { state: RootState }>(
  ...
  async (_, { getState }) => {
    ...
    await axiosRequest.delete(`users/sessions/`, {
